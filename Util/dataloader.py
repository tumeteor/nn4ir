...
class DataLoader(object):
    ...
    def __init__(self, pretrained=False, embedding = False, qsize="1k"):
        ...
        if qsize == "1k":
            ...
            self._data_config = cfg['DataConfig']
            self.log.info("using 1k queryset")
        self._d_handler = TextDataHandler(self._data_config['all_doc_path'], self._data_config['save_dir_data'], pretrained)
    ...
    def create_datasets2(self, dataset, labels, train_ratio, valid_ratio, test_ratio,
        ...
        Utilities.getIntancesFromQueries(train_queries, pairDict, train_qv, cache=True, h5py_path=h5py_path, type="train")
        Utilities.getIntancesFromQueries(valid_queries, pairDict, valid_qv, cache=True, h5py_path=h5py_path, type="valid")
        Utilities.getIntancesFromQueries(test_queries, pairDict, test_qv, cache=True, h5py_path=h5py_path, type="test")
    ...
    def create_datasets(self, dataset, labels, train_ratio, valid_ratio, test_ratio):
        ...
        train_dataset, train_labels, train_queries = Utilities.getIntancesFromQueries(train_queries, pairDict, train_qv)
        valid_dataset, valid_labels, valid_queries = Utilities.getIntancesFromQueries(valid_queries, pairDict, valid_qv)
        test_dataset, test_labels, test_queries = Utilities.getIntancesFromQueries(test_queries, pairDict, test_qv)
    ...
    def prepare_test_data(self):
        dts, lbl = self._r_datautil.loadGroundtruth()
        ...
        if self.pretrained:
            ...
            print(labels[:,0])
            return np.array(list(vocab_processor.transform(dataset))), labels, np.array(list(vocab_processor.transform(labels[:,0])))
        elif self.embedding:
            '''
            TODO
            '''
