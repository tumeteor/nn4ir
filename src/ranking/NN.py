...
class NN:
    ...
    def batch_data(self, batch_size, data, labels, queries):
        """
    ...
    @staticmethod
    def batch_data_pairwise(batch_size, data_left, data_right, labels, queries):
        """
        split data into smaller batch by the first tensor dimension
        :param batch_size:
        :param data: Tensor
    ...
    def train(self, graph, tf_train_dataset, tf_train_labels, tf_train_queries,
        ...
        with tf.Session(graph=graph, config=tf.ConfigProto(log_device_placement=True)) as session:
            ...
            vsteps, valid_data_batches, valid_label_batches, valid_query_batches = self.batch_data(
                data=self.valid_dataset,
                labels=self.valid_labels,
                queries=self.valid_queries,
                batch_size=NNConfig.batch_size)

            tsteps, test_data_batches, test_label_batches, test_query_batches = self.batch_data(data=self.test_dataset,
            ...
            for step in range(NNConfig.num_steps):
                ...
                feed_dict = {tf_train_dataset: batch_data, tf_train_labels: batch_labels,
                             tf_train_queries: batch_queries}
    ...
    def train_pairwise(self, graph, train_dataset, train_labels, train_queries, valid_dataset, valid_labels,
                       valid_queries,
        ...
        with tf.Session(graph=graph, config=tf.ConfigProto(log_device_placement=True)) as session:
            ...
            saver = tf.train.Saver()
            logger.info('Initialized')
            vsteps, valid_left_batches, valid_right_batches, valid_label_batches, valid_query_batches = NN.batch_data_pairwise(
                data_left=valid_data_left,
                data_right=valid_data_right,
                labels=valid_labels_new,
                queries=valid_data_queries,
                batch_size=NNConfig.batch_size)
            tsteps, test_left_batches, test_right_batches, test_label_batches, test_query_batches = NN.batch_data_pairwise(
